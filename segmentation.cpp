...
segmentation::segmentation(QWidget *parent , Qt::WindowFlags flags)
    ...
    for(int z = 0; z < imgSize[2]; z++)
        for(int x = 0; x < imgSize[0]; x++)
        {
            ...
            if(p[z][x] == 1)
            {
                ...
                {
                    initvoxelIndex[1] = y;
                    initOutputImage->SetPixel(initvoxelIndex, 255);
                }
            }
        }
        ...
        initWriter->SetFileName(initFileName);
        ...
        for(int z = 0; z < imgSize[2]; z++)
            for(int x = 0; x < imgSize[0]; x++)
            {
                ...
                if(p[z][x] == 1)
                {
                    ...
                    {
                        newvoxelIndex[1] = y;
                        outputImage->SetPixel(newvoxelIndex, 65535);
                    }
                }
            }
            ...
            strcpy(outputFileName, filePrefix.c_str());//string to char*
            initResultFileName = outputFileName;
            writer->SetFileName(outputFileName);
...
{
    emit executeAbnormalRegionVisualization();
}
...
void segmentation::on_morphologyOperationButton_clicked()
{
    ...
    reader->SetFileName( initFileName );
    reader->Update();
    inputImage = reader->GetOutput();
    ...
    for(int z = 0; z < imgSize[2]; z++)
        for(int y = 0; y < imgSize[1]; y++)
            ...
            reader->Update();
            inputImage = reader->GetOutput();
            orignalImgreader->SetFileName(inputFileName);
            ...
            for(int z = 0; z < imgSize[2]; z++)
                for(int y = 0; y < imgSize[1]; y++)
                    ...
                    emit returnInternalFileName(initResultFileName);
                    emit returnOutputFileName(outputFileName);
}

void segmentation::erosionOperation()
{
    ...
    for(int z = 0; z < imgSize[2]; z++)
        for(int y = 0; y < imgSize[1]; y++)
            ...
            //binaryErode->GetOutput()->GetPixel(index);
